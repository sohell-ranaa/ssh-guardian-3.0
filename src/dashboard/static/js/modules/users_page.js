...
(function() {
    ...
    async function loadUsers() {
        ...
        if (container) {
            container.innerHTML = `<div style="text-align: center; padding: 40px; color: ${TC.textSecondary};">Loading users...</div>`;
        }
        ...
        try {
            ...
            if (container) {
                container.innerHTML = `<div style="text-align: center; padding: 40px; color: ${TC.danger};">Failed to load users. Please try again.</div>`;
            }
        }
    }
    ...
    function renderUsers(usersList) {
        ...
        if (!usersList || usersList.length === 0) {
            container.innerHTML = `
                <div style="text-align: center; padding: 60px 20px; color: ${TC.textSecondary};">
                    ...
                    <h3 style="font-size: 18px; font-weight: 600; margin-bottom: 8px; color: ${TC.textPrimary};">No Users</h3>
                ...
                <table style="width: 100%; border-collapse: collapse;">
                    <thead>
                        <tr style="background: ${TC.surfaceAlt};">
                            <th style="text-align: left; padding: 14px 16px; font-size: 13px; font-weight: 600; color: ${TC.textSecondary}; border-bottom: 1px solid ${TC.border};">User</th>
                            <th style="text-align: left; padding: 14px 16px; font-size: 13px; font-weight: 600; color: ${TC.textSecondary}; border-bottom: 1px solid ${TC.border};">Email</th>
                            <th style="text-align: left; padding: 14px 16px; font-size: 13px; font-weight: 600; color: ${TC.textSecondary}; border-bottom: 1px solid ${TC.border};">Role</th>
                            ...
                            <th style="text-align: left; padding: 14px 16px; font-size: 13px; font-weight: 600; color: ${TC.textSecondary}; border-bottom: 1px solid ${TC.border};">Last Login</th>
        ...
        usersList.forEach((user, index) => {
            ...
            const statusColor = user.is_active ? TC.successDark : TC.danger;
            const statusText = user.is_active ? 'Active' : 'Inactive';

            const roleColors = {
                'Super Admin': TC.primary,
                'Admin': TC.purple,
                'Analyst': TC.teal,
                'Viewer': TC.textSecondary
            };
            const roleColor = roleColors[user.role_name] || TC.textSecondary;
                ...
                <tr style="${borderStyle}">
                    <td style="padding: 14px 16px; font-size: 14px; color: ${TC.textPrimary};">
                        <div style="display: flex; align-items: center; gap: 12px;">
                            <div style="width: 40px; height: 40px; border-radius: 50%; background: linear-gradient(135deg, ${TC.primary}, ${TC.primaryLight}); display: flex; align-items: center; justify-content: center; color: white; font-weight: 600; font-size: 16px;">
                                ${getInitials(user.full_name)}
                            </div>
                            <div>
                                <div style="font-weight: 500;">${user.full_name}</div>
                                <div style="font-size: 12px; color: ${TC.textSecondary};">ID: ${user.id}</div>
                            </div>
                        </div>
                    </td>
                    <td style="padding: 14px 16px; font-size: 14px; color: ${TC.textSecondary};">
                        <div style="display: flex; align-items: center; gap: 6px;">
                            ${user.email}
                            ${user.is_email_verified ? `<span title="Email Verified" style="color: ${TC.successDark};">✓</span>` : ''}
                        </div>
                    </td>
                    ...
                    <td style="padding: 14px 16px;">
                        <div style="display: flex; flex-direction: column; gap: 4px;">
                            <span style="display: inline-flex; align-items: center; gap: 6px;">
                                ...
                                <span style="font-size: 14px; color: ${TC.textSecondary};">${statusText}</span>
                            </span>
                            ${isLocked ? `<span style="font-size: 11px; color: ${TC.danger};">🔒 Locked</span>` : ''}
                        </div>
                    </td>
                    <td style="padding: 14px 16px; font-size: 14px; color: ${TC.textSecondary};">
                        ...
                        <div style="display: flex; gap: 8px; justify-content: center; flex-wrap: wrap;">
                            ...
                            ${isLocked ? `
                                <button class="btn-unlock-user" data-user-id="${user.id}"
                                        style="padding: 6px 12px; font-size: 12px; background: ${TC.warning}; color: ${TC.textPrimary}; border: none; border-radius: 4px; cursor: pointer;">
                                    Unlock
                                </button>
                            ` : ''}
                            <button class="btn-toggle-user" data-user-id="${user.id}" data-active="${user.is_active}"
                                    style="padding: 6px 12px; font-size: 12px; background: ${user.is_active ? TC.textSecondary : TC.successDark}; color: white; border: none; border-radius: 4px; cursor: pointer;">
            ...
            <div id="create-user-modal" class="modal-overlay" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); z-index: 1000; justify-content: center; align-items: center;">
                <div class="modal-content" style="background: ${TC.surface}; border-radius: 8px; width: 100%; max-width: 500px; max-height: 90vh; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.2);">
                    <div class="modal-header" style="padding: 20px 24px; border-bottom: 1px solid ${TC.border}; display: flex; justify-content: space-between; align-items: center;">
                        <h3 style="margin: 0; font-size: 18px; font-weight: 600; color: ${TC.textPrimary};">Create New User</h3>
                        <button id="create-user-close-btn" style="background: none; border: none; font-size: 24px; cursor: pointer; color: ${TC.textSecondary}; padding: 0; line-height: 1;">&times;</button>
                    </div>
                    <div class="modal-body" style="padding: 24px; overflow-y: auto;">
                        <div style="display: flex; flex-direction: column; gap: 20px;">
                            <div class="form-group">
                                <label style="display: block; margin-bottom: 6px; font-weight: 500; font-size: 14px; color: ${TC.textPrimary};">
                                    Full Name <span style="color: ${TC.danger};">*</span>
                                </label>
                            ...
                            <div class="form-group">
                                <label style="display: block; margin-bottom: 6px; font-weight: 500; font-size: 14px; color: ${TC.textPrimary};">
                                    Email <span style="color: ${TC.danger};">*</span>
                                </label>
                            ...
                            <div class="form-group">
                                <label style="display: block; margin-bottom: 6px; font-weight: 500; font-size: 14px; color: ${TC.textPrimary};">
                                    Password <span style="color: ${TC.danger};">*</span>
                                </label>
                                <div class="password-input-wrapper" data-password-field="create-user-password">
                            ...
                            <div class="form-group">
                                ...
                                <select id="create-user-role" style="width: 100%; padding: 10px 12px; border: 1px solid ${TC.border}; border-radius: 4px; font-size: 14px; box-sizing: border-box;">
                            ...
                            <div class="form-group">
                                <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                                    ...
                                    <span style="font-size: 14px; color: ${TC.textPrimary};">Active</span>
                                </label>
                            </div>
                        </div>
                    </div>
    ...
    function renderEditUserModal() {
        return `
            <div id="edit-user-modal" class="modal-overlay" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); z-index: 1000; justify-content: center; align-items: center;">
                <div class="modal-content" style="background: ${TC.surface}; border-radius: 8px; width: 100%; max-width: 500px; max-height: 90vh; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.2);">
                    <div class="modal-header" style="padding: 20px 24px; border-bottom: 1px solid ${TC.border}; display: flex; justify-content: space-between; align-items: center;">
                        <h3 id="edit-user-title" style="margin: 0; font-size: 18px; font-weight: 600; color: ${TC.textPrimary};">Edit User</h3>
                        <button id="edit-user-close-btn" style="background: none; border: none; font-size: 24px; cursor: pointer; color: ${TC.textSecondary}; padding: 0; line-height: 1;">&times;</button>
                    </div>
                    <div class="modal-body" style="padding: 24px; overflow-y: auto;">
                        <div style="display: flex; flex-direction: column; gap: 20px;">
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                <label style="display: block; margin-bottom: 6px; font-weight: 500; font-size: 14px; color: ${TC.textPrimary};">
                                    New Password <span style="color: ${TC.textSecondary}; font-weight: normal;">(leave blank to keep current)</span>
                                </label>
                                <div class="password-input-wrapper" data-password-field="edit-user-password">
                            ...
                            <div class="form-group">
                                ...
                                <select id="edit-user-role" style="width: 100%; padding: 10px 12px; border: 1px solid ${TC.border}; border-radius: 4px; font-size: 14px; box-sizing: border-box;">
