...
HIGH_RISK_THRESHOLD = 60  # ML risk score threshold for high_risk_detected
...
class EventEnricher:
    ...
    def enrich_event(self, event_id: int, source_ip: str,
                     ...
                     skip_ml: bool = False,
                     skip_threat_intel: bool = False) -> Dict:
        """
            ...
            try:
                ...
                if ml_manager:
                    ...
                    if ml_result.get('ml_available', False):
                        # Update auth_events with ML results
                        ...
                        self._log(f"✅ ML complete (risk: {ml_result.get('risk_score')}, type: {ml_result.get('threat_type')})")
                        self._update_processing_status(event_id, 'ml_complete')

                        # Trigger notifications based on ML results
...
def enrich_event(event_id: int, source_ip: str, verbose: bool = True) -> Dict:
    """
    ...
    return enricher.enrich_event(event_id, source_ip)
